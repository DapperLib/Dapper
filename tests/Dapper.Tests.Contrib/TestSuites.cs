    ...
    public class SqlServerTestSuite : TestSuite
    {
        ...
        static SqlServerTestSuite()
        {
            using (var connection = new SqlConnection(ConnectionString))
            {
                ...
                connection.Open();
                dropTable("Things");
                connection.Execute("CREATE TABLE Things (FirstId int not null, SecondId int not null, Name nvarchar(100) not null, Created DateTime null, PRIMARY KEY (FirstId, SecondId));");
                dropTable("Stuff");
