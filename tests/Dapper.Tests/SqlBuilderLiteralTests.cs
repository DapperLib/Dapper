...
namespace Dapper.Tests
{
    ...
    public sealed class SystemSqlClientSqlBuilderLiteralTests : SqlBuilderLiteralTests<SystemSqlClientProvider> { }
#if MSSQLCLIENT
    [Collection("MiscTests")]
    public sealed class MicrosoftSqlClientSqlBuilderLiteralTests : SqlBuilderLiteralTests<MicrosoftSqlClientProvider> { }
#endif
    public abstract class SqlBuilderLiteralTests<TProvider> : TestBase<TProvider> where TProvider : DatabaseProvider
    {
        [Fact]
        public void TestNullableGuidSupport()
        {
            ...
            guid = Guid.NewGuid();
            var guid2 = connection.Query<Guid?>($"select @{guid}").First();
...
#pragma warning restore 0649
        ...
        public void TestStructAsParam()
        {
            ...
            var car2 = connection.Query<CarWithAllProps>($"select @{car1.Name} Name, @{car1.Age} Age, @{car1.Trap} Trap").First();
        ...
        public void CheckComplexConcat()
        {
            ...
            connection.ExecuteNQ($"""
            create table #users16726709 (first_name varchar(200), last_name varchar(200))
            insert #users16726709 values (@{"Fred"},@{"Bloggs"})
            insert #users16726709 values (@{"Tony"},@{"Farcus"})
            insert #users16726709 values (@{"Albert"},@{"TenoF"})
            """);

            // Using Dapper
            Assert.Equal(2, connection.Query<object>($"""
                SELECT * FROM #users16726709
                WHERE (first_name LIKE CONCAT(@{term}, '%') OR last_name LIKE CONCAT(@{term}, '%'));
                """).Count());
            Assert.Equal(3, connection.Query<object>($"""
                SELECT * FROM #users16726709
                WHERE (first_name LIKE CONCAT('%', @{term}, '%') OR last_name LIKE CONCAT('%', @{term}, '%'));
                """).Count());
        }
        ...
        {
            var guid = Guid.NewGuid();
            var dog = connection.Query<Dog>($"select '' as Extra, 1 as Age, 0.1 as Name1 , Id = @{guid}");
        ...
        {
            var guid = Guid.NewGuid();
            var dog = connection.Query<Dog>($"select Age = @{(int?)null}, Id = @{guid}");
