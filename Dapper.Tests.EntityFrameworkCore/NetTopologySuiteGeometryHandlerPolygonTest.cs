...
{
    public class NetTopologySuiteGeometryHandlerPolygonTest : TestBase<SystemSqlClientProvider>
    {

        public NetTopologySuiteGeometryHandlerPolygonTest()
        {
            Handlers.RegisterGeometry();
        }

        [Fact]
        public void TestPolygonGeometrySelect()
        {
            var data = connection.Query<PolygonModel>("SELECT TOP 5 Id, CreateTime, Feature.Serialize() AS Feature FROM PolygonGeometrys");

            Assert.Equal(5, data.Count());
        }

        [Fact]
        public void TestPolygonGeometrySingleInfo()
        {
            var datum = connection.QueryFirstOrDefault<PolygonModel>("SELECT Id, CreateTime, Feature.Serialize() AS Feature FROM PolygonGeometrys WHERE Id = @id", new { id = new Guid("948902dd-6ac2-4ca4-abf9-7c9efcb859a5") });
        ...
        [Fact]
        public void TestPolygonGeometryInsert()
        {
            ...
            { SRID = 4326 };
            string sql = "INSERT INTO PolygonGeometrys (Id, CreateTime, Feature) VALUES (@id, @createTime, @feature)";
